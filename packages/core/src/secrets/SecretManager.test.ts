...
describe('SecretManager', () => {
  ...
  it('adds and retrieves a loader and provider', () => {
    ...
    manager = manager.addProvider('kube', new KubernetesSecretProvider({ name: 'my-secret' })) as SecretManager;
  ...
  it('adds a secret with full options object', () => {
    manager = manager
      ...
      .addProvider('kube', new KubernetesSecretProvider({ name: 'secret' }))
  ...
  it('adds a secret and prepares it', async () => {
    manager = manager
      ...
      .addProvider('kube', new KubernetesSecretProvider({ name: 'my-secret' }))
      .setDefaultLoader('memory')
      .setDefaultProvider('kube')
  ...
  it('prevents duplicate loader, provider, and secret', () => {
    ...
    manager = manager.addProvider('kube', new KubernetesSecretProvider({ name: 'my-secret' })) as SecretManager;
    expect(() => manager.addProvider('kube', new KubernetesSecretProvider({ name: 'my-secret' }))).toThrow(
  ...
  it('auto-assigns default loader/provider when only one is registered', async () => {
    manager = manager
      ...
      .addProvider('kube', new KubernetesSecretProvider({ name: 'auto-secret' }))
  ...
  it('throws when multiple providers but no default set', () => {
    manager = manager
      ...
      .addProvider('kube1', new KubernetesSecretProvider({ name: 's1' }))
      .addProvider('kube2', new KubernetesSecretProvider({ name: 's2' }))
  ...
  it('throws when multiple loaders but no default set', () => {
    manager = manager
      ...
      .addProvider('kube', new KubernetesSecretProvider({ name: 's' }))
